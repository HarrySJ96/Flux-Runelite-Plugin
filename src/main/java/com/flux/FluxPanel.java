...
public class FluxPanel extends PluginPanel {
	...
	private void addToDropdown(EntryConfig config) {
		dropdown.addItem(new ComboBoxIconEntry(new ImageIcon(ImageUtil.loadImageResource(getClass(), config.iconPath)), config.label, Optional.of(config.entry.name().toLowerCase())));
	}
	...
	private void activateButtonByLabel(String label) {
		footerButtons.stream().filter(btn -> btn.getText().trim().equals(label)).findFirst().ifPresent(this::activateFooterButton);
	}
	...
	private void setButtonGlow(String label, boolean glow) {
		footerButtons.stream().filter(btn -> btn.getText().trim().equalsIgnoreCase(label.trim())).findFirst().ifPresent(btn -> {
			btn.setGlowing(glow);
			if (!glow) {
				btn.repaint();
				footerPanel.repaint();
	...
	public BotmCard getBotmCard() {
		return botmCard;
	}

	public SotwCard getSotwCard() {
		return sotwCard;
	}

	public HuntCard getHuntCard() {
		return huntCard;
	}

	public AdminHubCard getAdminHubCard() {
		return adminHubCard;
	}
