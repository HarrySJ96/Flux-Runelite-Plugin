...
public class CompetitionFinder {
	...
	public Optional<CompetitionData> findHuntCompetition(int competitionId) {
		...
		return Optional.of(new CompetitionData(competitionId, details.get("title").getAsString(), startsAt, endsAt, null, huntData));
	}
	...
	private Optional<CompetitionData> fetchCompetitionData(int competitionId, EventType type, Instant startsAt, Instant endsAt) {
		...
		return Optional.of(new CompetitionData(competitionId, details.get("title").getAsString(), startsAt, endsAt, type == EventType.SOTW ? dataParser.parseSotwLeaderboard(details) : null, null));
	}
