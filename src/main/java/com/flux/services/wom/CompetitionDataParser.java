...
public class CompetitionDataParser {
	...
	public LinkedHashMap<String, Integer> parseSotwLeaderboard(JsonObject competitionDetails) {
		...
		try {
			JsonArray participants = competitionDetails.has("participations") ? competitionDetails.getAsJsonArray("participations") : new JsonArray();
			...
			for (JsonElement element : participants) {
				...
				String username = player != null && player.has("username") ? player.get("username").getAsString() : "";

				JsonObject progress = participant.has("progress") ? participant.getAsJsonObject("progress") : null;

				int xpGained = (progress != null && progress.has("gained")) ? progress.get("gained").getAsInt() : 0;
			...
			entries.stream().sorted((a, b) -> Integer.compare(b.xp, a.xp)).limit(TOP_PARTICIPANTS_COUNT).forEach(entry -> leaderboard.put(entry.username, entry.xp));
	...
	public HuntTeamData parseHuntTeamData(JsonObject competitionDetails) {
		try {
			...
			for (JsonElement element : participants) {
				...
				JsonObject player = participant.getAsJsonObject("player");
				String username = (player != null && player.has("displayName")) ? player.get("displayName").getAsString() : "";

				String teamName = participant.has("teamName") ? participant.get("teamName").getAsString() : "";

				JsonObject progress = participant.has("progress") ? participant.getAsJsonObject("progress") : null;

				double ehb = (progress != null && progress.has("gained")) ? progress.get("gained").getAsDouble() : 0.0;

				if (teamName.equals(teamNames.team1)) {
					team1Participants.add(new HuntParticipant(username, ehb));
				} else {
					if (teamName.equals(teamNames.team2)) {
			...
			return new HuntTeamData(teamNames.team1, teamNames.team2, team1Top10, team2Top10, (int) Math.round(team1Total), (int) Math.round(team2Total));
	...
	private LinkedHashMap<String, Double> getTopParticipants(List<HuntParticipant> participants) {
		...
		participants.stream().sorted((a, b) -> Double.compare(b.ehb, a.ehb)).limit(TOP_PARTICIPANTS_COUNT).forEach(p -> top10.put(p.username, p.ehb));
