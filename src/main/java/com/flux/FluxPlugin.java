...
@Slf4j
@PluginDescriptor(name = "Flux", description = "A plugin used to keep track of clan events.", tags = {"flux", "cc", "hunt", "pass", "event", "clan"})
public class FluxPlugin extends Plugin {
	...
	private void initializePanel() {
		...
		uiNavigationButton = NavigationButton.builder().tooltip("Flux").icon(icon).priority(config.menuPriority()).panel(panel).build();
	...
	private void initializeServices() {
		...
		competitionScheduler = new CompetitionScheduler(configManager, apiClient, finder, configUpdater);

		configParser = new GoogleSheetParser(configManager, GoogleSheetParser.SheetType.CONFIG, this::handleConfigUpdate, true, okHttpClient);
	...
	public void onConfigChanged(ConfigChanged event) {
		...
		if (key.equals("hunt_team_1_name") || key.equals("hunt_team_2_name") || key.equals("hunt_team_1_color") || key.equals("hunt_team_2_color")) {
			if (panel != null && panel.getHuntCard() != null) {
	...
	public void onChatMessage(ChatMessage event) {
		...
		if (loginMessage != null && event.getType() == ChatMessageType.BROADCAST && event.getMessage().contains(loginMessage)) {
